...
const Footer = () => {
	return (
		<div className='bg-white text-black	border-t border-gray-300'>
						...
						<nav className='space-y-3'>
							{headerLinks.map((link) => (
								<Link
									key={link.title}
									href={link.link}
									className='block text-gray-800 hover:text-gray-600 text-sm transition-colors duration-200 hover:underline'
								>
									{link.title}
								</Link>
							))}
					...
					<div className='space-y-4'>
						...
						<div className='space-y-3'>
							{regions.map((region) => (
								<p
									key={region.title}
									className='text-gray-800 text-sm cursor-default'
								>
									{region.title}
								</p>
							))}
			...
			<div className='border-t border-white/20'>
				<div className='container mx-auto px-4 sm:px-6 lg:px-8 py-6'>
					<div className='flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-gray-800'>
						<p>
							© {new Date().getFullYear()} Africa Tech Business. All rights
							reserved.
						</p>
						<p>
							Developed by{" "}
