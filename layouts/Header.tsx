			...
			<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
				<nav className='flex items-center justify-between md:h-[120px] h-24'>
					...
					<div className='hidden md:flex items-center gap-3 lg:gap-4'>
						...
						>
							<button className='cursor-pointer bg-black text-white rounded-lg shadow-md px-3 py-2 lg:px-5 lg:py-3 text-sm lg:text-base font-medium hover:bg-gray-50 transition-colors duration-200'>
								Get In Touch
							</button>
						</Link>
					</div>
				...
				>
					<div className='px-2 pt-2 pb-4 space-y-1 bg-white border-t border-white/20'>
								...
								{link.dropdownItems ? (
									<>
										...
										<button
											...
											className='flex justify-between items-center w-full px-3 py-3 text-base font-medium text-black hover:bg-white/10 rounded-md transition-colors duration-200'
											aria-expanded={openDropdown === link.title}
											...
											>
												{link.dropdownItems.map((item) => (
													<Link
														...
														className='block px-3 py-2 text-sm font-medium text-black/90 hover:bg-white/10 rounded-md transition-colors duration-200'
													>
														{item.title}
													</Link>
												))}
						...
						<div className='pt-3 border-t border-white/20 mt-3'>
							<div className='flex items-center gap-3 px-3'>
								<button
									className='flex-1 flex items-center justify-center gap-2 bg-white/10 text-black rounded-lg px-4 py-3 hover:bg-white/20 transition-colors duration-200'
									onClick={closeMobileMenu}
								>
									<Icon
										icon={"ic:baseline-search"}
