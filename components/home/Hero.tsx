...
const Hero = () => {
	...
	if (posts.length === 0) {
		return (
			<section className='relative h-[80vh] bg-gradient-to-br from-white to-gray-500 flex items-center justify-center'>
				<div className='text-center'>
					<h1 className='text-4xl md:text-6xl font-bold text-black mb-4'>
						Stories That Shape Tech In Africa
					</h1>
					<p className='text-xl text-gray-600 max-w-2xl mx-auto'>
						Bringing you authentic voices, compelling narratives, and
						insightful analysis
					</p>
				</div>
			</section>
						...
						<AnimatePresence mode='wait'>
							...
							>
										...
										<div className='flex flex-wrap gap-3 mb-6'>
											{currentPost.categories
												...
												.map((category) => (
													<span
														key={category._id}
														className='px-4 py-2 bg-white text-black text-sm font-semibold rounded-full backdrop-blur-sm'
													>
														{category.title}
													</span>
												))}
								...
								<Link
									...
									className='inline-flex items-center gap-3 bg-white hover:bg-gray-400 text-black px-8 py-4 rounded-lg text-lg font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-xl'
								>
									Read Full Story
									<Icon
